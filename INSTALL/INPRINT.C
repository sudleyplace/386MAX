/*' $Header:   P:/PVCS/MAX/INSTALL/INPRINT.C_V   1.1   05 Sep 1995 16:26:18   HENRY  $ */
/******************************************************************************
 *									      *
 ...
 * INPRINT.C								      *
 *									      *
 * Printer routines for INSTALL.EXE					      *
 *									      *
 ******************************************************************************/
...
#define INPRINT_C			/* ..to select message text */
#include "instext.h"            /* MAX message text */
#undef INPRINT_C
...
void killPrinting( void )
{
	...
	visible_mouse (0); // Only show mouse cursor when getting input
	restoreButtons( );
	if( saveBehind ) {
...
#endif
		if( (long) time(NULL) > timeOut ) break;
	}
...
/**
*
* Name		printEditor - Print all the help in a help file.
*
