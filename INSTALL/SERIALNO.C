...
unsigned char
weight_digit (char *inpstr)

{ unsigned int i, j, len;
  register unsigned char c, s;

  len = strlen (inpstr);	// Loop through each character in the string
...
{ unsigned char s;
  ...
  return ((unsigned char) ('0' + strlen (d5[s])));
...
unsigned char *
decode_serialno (unsigned char *inpstr)

{ unsigned int i, len, nprec;
  unsigned char c, tmpstr[MAXSTR + 1];

  // Re-order digits of input string to unconfuse the program
  len = strlen (inpstr);
  // Ensure there's something to process
  if (len < 2)			// We need at least two check digits
//////BEEP ();
