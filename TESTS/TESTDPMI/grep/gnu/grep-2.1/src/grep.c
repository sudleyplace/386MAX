...
/* Internationalization. */
  ...
  while ((opt = getopt_long(argc, argv,
	...
	break;
      case 'e':
	cc = strlen(optarg);
	keys = xrealloc(keys, keycc + cc + 1 + keyfound);
	if (keyfound)
	  keys[keycc++] = '\n';
	...
	if (!fp)
	  fatal(optarg, errno);
	for (keyalloc = 1; keyalloc <= keycc + keyfound; keyalloc *= 2)
	  ;
	...
	if (desc < 0)
	...
	else
	  {
	    ...
	    if (!isatty(desc))
	      SET_BINARY(desc);
#endif
	    filename = desc == 0 ? _("(standard input)") : argv[optind];
	    count = grep(desc);
	    if (count_matches)
	      {
		if (out_file)
