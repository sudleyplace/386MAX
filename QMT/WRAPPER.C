...
void QuoteExpand (char *CurrLine);
	...
	for ( i = 0 ; i < lInFileSize ; )
		{
			...
			if ( hpbInFile[i] == SIGNAL_CHAR )
				...
				if( (CharPos > iWidth ) || hpbInFile[i-1] == '\r' ) // Have we blown the width?
					{
					...
					while( CharPos <= iWidth )
						CurrLine[CharPos++]=' ';                // Blank out rest of line
					
					CurrLine[ CharPos ] = '\0'; // Write in a NULL

					PosLastSpace = 0;			// Re-init some vars for next time
					IofLastSpace = 0;			// . . .
					CharPos 	 = 0;			// . . .

					QuoteExpand (CurrLine);
...
// Expand single quotes to two of them
void QuoteExpand (char *CurrLine)

{
	char CurrLine2[256];	// Temp line buffer
	int i, j;

	strcpy (CurrLine2, CurrLine);
	for (i = j = 0; i < sizeof (CurrLine2); i++, j++)
	switch (CurrLine2[i])
	{
		case '\0':
			CurrLine[j] = CurrLine2[i];
			return;
