/*' $Header:   P:/PVCS/MAX/MAXIMIZE/INTMSG.C_V   1.0   05 Sep 1995 16:33:12   HENRY  $ */
/******************************************************************************
 *									      *
...
#define INTMSG_C		/* ..to select message text */
...
static INTMSGWIN IntMsg;
static char IntMsgColors[2];

#define DEFAULTINDENT 10
static int IntMsgIndent = DEFAULTINDENT;

static int IntMsgQref[3] = { HELP_QPHASE1, HELP_QPHASE2, HELP_QPHASE3 };


/*************************** INTMSG_SETUP **********************************/

void intmsg_setup(void) /* Set up the window for dointmsg() */
{
	int area;
...
{
	/* This stuff usually gets stepped on ... */
...
void _cdecl do_intmsg(char *format,...) /* Print intermediate message */
{
	...
	for (kk = 0; StrList[kk]; kk++) {
		if (stricmp(p,StrList[kk]) == 0) break;
	}
	if (StrList[kk]) {
		/* If leading token matches our list */
		/* write out in white with trailing colon */
		strcpy (left, p);
		strcat (left, ":");
		nc = strlen(left);
		if (nc < 10) {
			memset(left+nc,' ',10-nc);
			left[10] = '\0';
		}
